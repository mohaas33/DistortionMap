...
ff= open("./run_all_avg_pp_jobs.sh","w+")
ff.write("#!/usr/bin/bash"+"\n")

for i in range(0,1200):
    filename = "./macros/run_files_avg_pp_{}_{}.sh".format(i*5,(i+1)*5)
    f= open(filename,"w+")
    f.write("#!/usr/bin/bash"+"\n")
    f.write("source macros/run_files_avg_pp.sh {} {}".format(i*5,(i+1)*5)+"\n")
    f.close
    filename_job = "./macros/condor_run_files_avg_pp_{}_{}.job".format(i*5,(i+1)*5)
    ...
    for lines in introduction:
        ...
        if n_line==3:
            ...
            f_job.write("Executable      = macros/run_files_avg_pp_{}_{}.sh".format(i*5,(i+1)*5)+"\n")
            f_job.write(""+"\n")
            f_job.write(""+"\n")
            f_job.write("# The argument to pass to the executable."+"\n")
            f_job.write("Arguments       = \"run job pp avg {} {}\"".format(i*5,(i+1)*5)+"\n")
        if n_line==38:
            f_job.write("# The job's stdout is sent to this file."+"\n")
            f_job.write("Output          = /sphenix/user/shulga/Work/IBF/DistortionMap/Out/myjob_avg_pp_{}_{}.out".format(i*5,(i+1)*5)+"\n")
            f_job.write(""+"\n")
            f_job.write("# The job's stderr is sent to this file."+"\n")
            f_job.write("Error           = /sphenix/user/shulga/Work/IBF/DistortionMap/Out/myjob_avg_pp_{}_{}.err".format(i*5,(i+1)*5)+"\n")
            f_job.write(""+"\n")
            f_job.write("# The condor log file for this job, useful when debugging."+"\n")
            f_job.write("Log             = /sphenix/user/shulga/Work/IBF/DistortionMap/Out/condor_avg_pp_{}_{}.log".format(i*5,(i+1)*5)+"\n")
            f_job.write(""+"\n")
