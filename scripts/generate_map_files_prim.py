...
for i in range(0,20):
    ...
    for lines in introduction:
        ...
        if n_line==3:
            ...
            f_job.write("Executable      = macros/run_files_map_prim_{}.sh".format(i)+"\n")
            f_job.write(""+"\n")
            f_job.write(""+"\n")
            f_job.write("# The argument to pass to the executable."+"\n")
            f_job.write("Arguments       = \"run primary job bias {} \"".format(i)+"\n")
        if n_line==38:
            f_job.write("# The job's stdout is sent to this file."+"\n")
            f_job.write("Output          = /sphenix/user/shulga/Work/IBF/DistortionMap/Out/myjob_prim_{}.out".format(i)+"\n")
            f_job.write(""+"\n")
            f_job.write("# The job's stderr is sent to this file."+"\n")
            f_job.write("Error           = /sphenix/user/shulga/Work/IBF/DistortionMap/Out/myjob_prim_{}.err".format(i)+"\n")
            f_job.write(""+"\n")
            f_job.write("# The condor log file for this job, useful when debugging."+"\n")
            f_job.write("Log             = /sphenix/user/shulga/Work/IBF/DistortionMap/Out/condor_prim_{}.log".format(i)+"\n")
            f_job.write(""+"\n")
