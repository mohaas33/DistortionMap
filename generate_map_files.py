...
for i in range(0,30):
    ...
    f.write("root.exe -l -b -q macros/run_analysis.C\({}\)".format(i*10)+"\n")
    f.close

    filename_job = "./macros/condor_run_map_files_{}.job".format(i*10)
    ...
    for lines in introduction:
        ...
        if n_line==3:
            ...
            f_job.write("Executable      = macros/run_files_map_{}.sh".format(i*10)+"\n")
            f_job.write(""+"\n")
            f_job.write(""+"\n")
        ...
        if n_line==38:
            ...
            f_job.write("Output          = /sphenix/user/shulga/Work/IBF/DistortionMap/Out/myjob_{}.out".format(i*10)+"\n")
            f_job.write(""+"\n")
            f_job.write("# The job's stderr is sent to this file."+"\n")
            f_job.write("Error           = /sphenix/user/shulga/Work/IBF/DistortionMap/Out/myjob_{}.err".format(i*10)+"\n")
            f_job.write(""+"\n")
            f_job.write("# The condor log file for this job, useful when debugging."+"\n")
            f_job.write("Log             = /sphenix/user/shulga/Work/IBF/DistortionMap/Out/condor_{}.log".format(i*10)+"\n")
            f_job.write(""+"\n")
